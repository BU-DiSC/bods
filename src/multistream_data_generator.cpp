...
std::vector<BoDSConfig> parse_args(int argc, char *argv[]) {
    ...
    for(int i = 0; i < partitions; i++) {
        config.totalNumbers = tbl["partition"][i]["N"].value_or(10000);
        config.domain_right = tbl["global"]["D"].value_or(30000);
        config.K = tbl["partition"][i]["K"].value_or(0);
        config.L = tbl["partition"][i]["L"].value_or(0);
        config.seedValue = tbl["partition"][i]["S"].value_or(0);
        config.outputFile = tbl["global"]["o"].value_or("/workloads/createdata_partitions_3");
        config.alpha = tbl["partition"][i]["a"].value_or(0);
        config.beta = tbl["partition"][i]["b"].value_or(0);
        config.window_size = tbl["partition"][i]["W"].value_or(1);
        config.payload_size = tbl["partition"][i]["P"].value_or(0);
        config.fixed_window = tbl["partition"][i]["F"].as_boolean();
        config.binary = tbl["partition"][i]["B"].as_boolean();
        config.start_index = tbl["partition"][i]["I"].value_or(0);
